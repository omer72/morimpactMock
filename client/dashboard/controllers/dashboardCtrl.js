        ...
        function init(){
            ...
            if (!userPrefObject){
                var userPref =
                    {'userId': $rootScope.currentUser._id,
                        "robotPicName" : "1101",
                        "background-picture" : "0003mainBGVars_daylightSnowing1364x768"}
                    ;
                userPrefObject = {};
        ...
        dc.clickMenu = function(ev,value){
            ...
            switch (value.scopeName){
                case "generalPlace":
                    ...
                    break;
                case "talks":
                    dc.gp = Talks.find({userId:{"$in":dc.userId}}).fetch();
                    dc.show_modal = true;
                    break;
                case "polite":
                    dc.gp = Polite.find({userId:{"$in":dc.userId}}).fetch();
                    dc.show_modal = true;
                    break;
                case "sales":
                    dc.gp = Sales.find({userId:{"$in":dc.userId}}).fetch();
                    dc.show_modal = true;
                    break;
                case "leaderBoard":
